  ...
  if drawBoardCounter % 10 = 0 then
    let (lx, ly) =
      ...
      |> Seq.fold 
        (fun (mx, my) kv -> 
          let (x,y) = kv.Key
          ((if x < mx then x else mx), (if y < my then y else my))
        )
        (0,0)
    board
    |> Seq.iter (fun kv ->
      System.Console.SetCursorPosition ((fst kv.Key) - lx, (snd kv.Key) - ly)
      match kv.Value with
      | 0 ->
      ...
      | x -> failwith (sprintf "unknown tile type: %A" x)
      printf " "
    )

    System.Console.SetCursorPosition (cx - lx, cy - ly)
    System.Console.BackgroundColor <- System.ConsoleColor.Blue
    printf " "

  drawBoardCounter <- drawBoardCounter + 1
...
let main argv =
  ...
  let board = Dictionary<int * int, int>()
  let mutable currentPos = 0,0
  let mutable currentHeading = North
  ...
  let outputf s =
    match s with
    | "0" -> //wall
      board.[move currentHeading currentPos] <- 0
      currentHeading <- rotateRight currentHeading
    | "1" -> //move
      currentPos <- move currentHeading currentPos
      board.[currentPos] <- 1
      currentHeading <- rotateLeft currentHeading
    ...
    | "2" -> //move+goal
      ...
      board.[currentPos] <- 2
      moveCounter <- moveCounter + 1
    | x -> failwith (sprintf "unknown output %A" x)

  
  let inputf () =
    // printfn "%A %A" currentPos currentHeading
    drawBoard currentPos board
    directionToValue currentHeading
    |> string
    |> Some
  ...
  // board is completly mapped. Find distance to exit
  let openNodes = Queue<int * int * int>()
  let visitedNodes = HashSet<int * int>()
  openNodes.Enqueue (0,0,0)
  ...
  while openNodes.Count > 0 do
    ...
    |> List.filter (fun np ->
      ...
      if s && b = 2 then printfn "distance to exit = %A" (d+1)
