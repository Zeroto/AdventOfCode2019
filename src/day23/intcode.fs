...
let run input output memory pos relativeBase log =
  let result = handleOpcode input output memory pos relativeBase log
  match result with
  | Finished -> (true, pos, relativeBase)
  | MoveForward x -> (false, pos+x, relativeBase) // run input output memory (pos+x) relativeBase log
  | WaitForInput -> (false, pos, relativeBase)
  | SetBase x -> (false, pos + bigint 2, x)
  | Jump x -> (false, x, relativeBase)

let runProgram program inputf outputf log =
  let mutable finished = false
  let mutable ip = bigint 0
  let mutable relBase = bigint 0
  let log _ = ()
  while not finished do
    let (f, p, b) = run inputf outputf program ip relBase log
    finished <- finished || f
    ip <- p
    relBase <- b