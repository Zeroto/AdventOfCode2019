...
let testInput = "R75,D30,R83,U83,L12,D49,R71,U7,L72
U62,R66,U55,R34,D71,R55,D58,R83"
let testInput2 = "R98,U47,R26,D63,R33,U87,L62,D20,R33,U53,R51
U98,R91,D20,R16,D67,R40,U7,R15,U6,R7"
let testInput3 = "R8,U5,L5,D3
U7,R6,D4,L4"

type IntersectResult =
  | Intersection of (float * float)
  | NoIntersection
...
let intersectTest (line1: Line) (line2: Line) =
  ...
  if determinator = 0. then
    ...
    if (t<0. || t>1. || u<0. || u>1.) then
      ...
      // calculate point
      printfn "%A, %A" line1 line2
      let (dirx, diry) = ((l1x2 - l1x1) * t, (l1y2 - l1y1)*t)
      let posx, posy = l1x1 + dirx, l1y1 + diry
      Intersection (posx, posy)
...
let main argv =
  ...
  let wires =
    ...
    |> Array.map 
      ( Array.fold
          (fun (wires, (x, y)) i ->
            let op = i.[0]
            let value = i.Substring(1) |> float
            match op with
            | 'U' ->
              (wires @ [(x,y, x, y+value)], (x, y+value))
            | 'R' -> 
              (wires @ [(x,y, x+value, y)], (x+value, y))
            | 'D' -> 
              (wires @ [(x,y, x, y-value)], (x, y-value))
            | 'L' -> 
              (wires @ [(x,y, x-value, y)], (x-value, y))
            | x -> failwith (sprintf "Unknown instruction: %A" x)
          ) 
          ([], (0.,0.))
        >> fst
      )
  ...
  let intersections = 
    ...
    |> List.collect (fun w1 ->
      wires.[1]
      |> List.choose (fun w2 ->
        let result = intersectTest w1 w2
        match result with
        | NoIntersection -> None
        | Intersection pos -> Some pos
      )
    )
    |> List.filter (fun i -> i <> (0.,0.))
  ...
  let rec goDir (x, y) cdist dir length m =
    if length = 0 then ((x,y), cdist, m)
    else
      let newPos =
        match dir with
        | 'U' ->
      ...
      match Map.tryFind newPos m with
      | Some ed ->
        goDir newPos (cdist+1) dir (length-1) m
      | None ->
        goDir newPos (cdist+1) dir (length-1) (Map.add newPos (cdist+1) m)

  let walkDistanceMaps =
    wireInstructions
    |> Array.map 
      ( Array.fold
          (fun (m, pos, cdst) i ->
            let op = i.[0]
            let value = i.Substring(1) |> int
            let (npos, ndist, nm) = goDir pos cdst op value m
            (nm, npos, ndist)
          ) 
          (Map.empty |> Map.add (0,0) 0, (0,0), 0)
        >> (fun (x,_,_) -> x)
      )

  printfn "%A" walkDistanceMaps

  let distances =
    intersections
    |> List.map (fun (x,y) -> Map.find (int <| round x, int <| round y) walkDistanceMaps.[0] + Map.find (int <| round x, int <| round y) walkDistanceMaps.[1])
    |> List.sort
